...
public class RainbowZombieCongaLine {
	...
	public void jumpInTheLine(Zombie dancer, int position) {
		ArrayList<Zombie> zombiesToAdd = new ArrayList<Zombie>();
		for (int i = congaLine.size() - 1; i >= position; i--) {
			zombiesToAdd.add(congaLine.getTail().getValue());
			congaLine.remove(i);
		}
		congaLine.add(dancer);
		for (Zombie zombie : zombiesToAdd) {
	...
	public void everyoneOut(Zombie dancer) {
		...
		for (int i = congaLine.size() - 1; i >= 0; i--) {
			if (currentNode.getValue().getZombieHatColor() == hatColor) {
				congaLine.remove(i);
			}
	...
	public void youAreDone(Zombie dancer) {
		ZombieHatColor hatColor = dancer.getZombieHatColor();
		Node<Zombie> currentNode = congaLine.getTail();
